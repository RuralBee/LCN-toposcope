...
class Newlink(object):
    def __init__(self, part, prefix_name):
        ...
        self.asn = asn_attr("./202107data/asn-list.txt")
        all_VP = set(open('./hidden-data/chooseVP' + self.part + '.txt', 'r').read().split('\n'))
        self.fullVP = set(open('./hidden-data/fullVP.txt', 'r').read().split('\n'))
        self.fullAS = eval(open('./hidden-data/chooseAS.txt', 'r').read())
        self.fullVP1 = all_VP & self.fullVP
        self.fullAS1 = self.fullAS - self.fullVP

        self.ba = BasicAtts('./hidden-data/asrel' + self.part + '.txt')
        self.ba.calPr()
    ...
    def getEdgeAmount(self):
        if os.path.exists("./hidden-data/edgeamount" + self.part + ".pkl"):
            self.edgeamount = joblib.load("./hidden-data/edgeamount" + self.part + ".pkl")
        else:
            with open('./hidden-data/asprefix' + self.part + '.txt') as f:
                for line in f:
                    path = line.strip().split('&')[0].split('|')
                    n = len(line.strip().split('&')[1].split("%"))
                    for i in range(len(path) - 1):
                        asn1, asn2 = path[i], path[i + 1]
                        self.edgeamount[(asn1, asn2)] += n
                        self.edgeamount[(asn2, asn1)] += n
            joblib.dump(self.edgeamount, "./hidden-data/edgeamount" + self.part + ".pkl")

    def getTriple(self):
        with open('./hidden-data/triplet_miss' + self.part + '.txt') as f:
            for line in f:
                if line == '\n':
                    continue
                ...
                for t in tri:
                    t = t.split('|')
                    if self.ba.graph.degree(t[0]) > self.ba.graph.degree(t[2]) or (
                            self.ba.graph.degree(t[0]) == self.ba.graph.degree(t[2]) and int(t[0]) > int(t[2])):
                        t = (t[2], t[1], t[0])
                    self.triple.add(tuple(t))

    def assignPosition(self):

        self.cnt = self.init_variable("./hidden-data/cnt"+ self.part + ".pkl", defaultdict(int))
        self.prev = self.init_variable("./hidden-data/prev"+ self.part + ".pkl", defaultdict(lambda: defaultdict(int)))
        self.next = self.init_variable("./hidden-data/next"+ self.part + ".pkl", defaultdict(lambda: defaultdict(int)))
        self.position = self.init_variable("./hidden-data/position."+ self.part + "pkl", defaultdict(lambda: defaultdict(int)))
        # self.seliver = defaultdict(lambda: defaultdict(set))
        self.seliver = self.init_variable("./hidden-data/seliver"+ self.part + ".pkl", defaultdict(lambda: defaultdict(set)))
        if os.path.exists("./hidden-data/cnt"+ self.part + ".pkl"):
            return
        report = 1
        with open('./hidden-data/asprefix' + self.part + '.txt') as f:
            for line in f:
                ...
                if line == '\n':
                    continue
                path = line.strip().split('&')[0].split('|')
                prefix = line.strip().split('&')[1].split("%")
                ...
                for i in range(len(path) - 2):
                    ...
                    if i == len(path) - 3:
                        ...
                        next = path[i + 3]
                    p, v = i, len(path) - 3 - i
                    if (asn1 in (self.fullVP | self.fullAS - self.fullVP1) or asn3 in (
                            self.fullVP | self.fullAS - self.fullVP1)) or (asn1, asn2, asn3) in self.triple or (
                    asn3, asn2, asn1) in self.triple:
                        if asn1 not in self.ba.graph.nodes() or asn2 not in self.ba.graph.nodes() or asn3 not in self.ba.graph.nodes():
                            continue
                        if self.ba.graph.degree(asn1) > self.ba.graph.degree(asn3) or (
                                self.ba.graph.degree(asn1) == self.ba.graph.degree(asn3) and int(asn1) > int(asn3)):
                            asn1, asn3 = asn3, asn1
                            prev, next = next, prev
                            p, v = v, p
                        ...
                        for i in prefix:
                            self.seliver[(asn1, asn2, asn3)][path[-1]].add(i)
        for tri in self.seliver:
            for origin in self.seliver[tri]:
                self.seliver[tri][origin] = len(self.seliver[tri][origin])

        joblib.dump(dict(self.cnt), "./hidden-data/cnt" + self.part + ".pkl")
        joblib.dump(dict(self.prev), "./hidden-data/prev" + self.part + ".pkl")
        joblib.dump(dict(self.next), "./hidden-data/next" + self.part + ".pkl")
        joblib.dump(dict(self.position), "./hidden-data/position" + self.part + ".pkl")
        joblib.dump(dict(self.seliver), "./hidden-data/seliver" + self.part + ".pkl")
    ...
    def xgboostInfer(self):
        ...
        for tri in self.cnt:
            ...
            x = []
            try:
                x.extend(self.asn.get_feature(tri))
            except:
                continue
            ...
            x.extend([len(set(self.ba.graph.neighbors(tri[0])) & (self.ba.clique | self.ba.tier2)),
                      len(set(self.ba.graph.neighbors(tri[2])) & (self.ba.clique | self.ba.tier2))])
        ...
        model = XGBClassifier(eta=0.2, min_child_weight=4, gamma=0, n_estimators=1000, max_depth=22,
        ...
        ftri = open('./hidden-data/futher' + self.part + '.txt', 'w')
        for i in range(len(predictions)):
            if predictions[i] == 1:
                ftri.write(pred_order[i][0] + '|' + pred_order[i][1] + '|' + pred_order[i][2] + '\n')
        ftri.close()
    def init_variable(self, file_name, type):
