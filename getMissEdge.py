...
for part in ['0', '1']:
    partial_VP = set(open('./hidden-data/chooseVP' + part + '.txt', 'r').read().split('\n'))
    fullVP = set(open('./hidden-data/fullVP.txt', 'r').read().split('\n'))
    fullVP1 = partial_VP & fullVP
    graph1 = nx.Graph()
    graph2 = nx.Graph()
    with open('./hidden-data/asrel' + part + '.txt') as f:
        for line in f:
            if line.startswith('#'):
                continue
            [asn1, asn2, rel] = line.strip().split('|')
            graph1.add_edge(asn1, asn2)
    with open('./basic_data/asrel.txt') as f:
        for line in f:
            if line.startswith('#'):
                continue
            [asn1, asn2, rel] = line.strip().split('|')
    ...
    pos = defaultdict(set)
    cnt = 0
    with open('./hidden-data/aspaths' + part + '.txt') as f:
        for line in f:
            if len(line.strip()) == 0:
                continue
    ...
    fout = open('./hidden-data/triplet_miss' + part + '.txt', 'w')
    for key in path:
